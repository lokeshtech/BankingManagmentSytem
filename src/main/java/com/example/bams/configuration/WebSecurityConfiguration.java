package com.example.bams.configuration;
...
import com.example.bams.service.MyUserDetailsService;
...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/").permitAll().antMatchers(loginPage).permitAll()
				...
				.antMatchers("/employee/**").hasAuthority("EMPLOYEE").anyRequest().authenticated().and().csrf()
				.disable().formLogin().loginPage(loginPage).loginPage("/").failureUrl("/login?error=true")
				.defaultSuccessUrl("/home", true).usernameParameter("user_name").passwordParameter("password").and()
